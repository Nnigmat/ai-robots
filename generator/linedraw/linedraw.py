...
rm_logs = False
export_svg = True
stroke_colors = ['white', 'black', 'yellow', 'green', 'blue']
...
def get_color():
    return '#' + ''.join([hex(np.random.randint(0, 255)).lstrip('0x') for i in range(3)])
...
def sketch(path):
    ...
    possible = [path, "images/"+path, "images/"+path +
                ".jpg", "images/"+path+".png", "images/"+path+".tif", "/home/nnigmat/Code/University/Thesis/project/linedraw/images/"+path]
    for p in possible:
        try:
            IM = Image.open(p)
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', dest='input_path',
                        default='lenna.png', action='store', nargs='?', type=str,
                        help='Input path')
    ...
    parser.add_argument('--show-lines', dest='show_lines',
                        const=not show_lines, default=show_lines, action='store_const', help='Show array of lines')
    parser.add_argument('--rm-logs', dest='rm_logs',
