...
func priority(robot, speed, target, location, close_robots, width, height, close_robots_changed, path_length):
	...
	astar.Init(width, height, blocked_points)
	
	var limited_target = (target - location).normalized() * 10
#	print(astar.FindPath(location.x, location.z, limited_target.x, limited_target.z))
	var next = astar.FindPath(location.x, location.z, limited_target.x, limited_target.z)
	
	if not next.empty():
		return Vector3(next[0].x, 0, next[0].y)
